...
Every request field must contain `criticalFieldCategory`, using `null` only after deciding that no profile in [material-field-profiles.json](material-field-profiles.json) applies.

For a material field, `constraintEvidence` must contain every dimension configured for its profile. Each dimension contains `documented`, `observed`, `verdict`, and `evidenceIds`. An executed verdict requires evidence; `BLOCKED` or `NOT_EXECUTED` requires a concrete reason in `observed`. `probeDimensions` lists only dimensions actually executed and must point to executed entries in `constraintEvidence`.
